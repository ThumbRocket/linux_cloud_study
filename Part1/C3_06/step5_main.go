...
func run() {
	...
	cmd.SysProcAttr = &syscall.SysProcAttr{
		...
		// syscall.CLONE_NEWPID : psid 분리 | syscall.CLONE_NEWNS : mount ns 추가
	}
...
func child() {
	...
	*/
	// 루트 파일시스템 변경
	// syscall.Chroot("/tmp/ubuntu") 					// chroot /tmp/ubuntu
	// syscall.Chdir("/")								// cd /
	// syscall.Mount("proc", "proc", "proc", 0, "")		// mount -t proc proc /proc
	// defer syscall.Unmount("proc", 0)					// umount /proc
